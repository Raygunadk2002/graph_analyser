...
def compute_tvccf_peaks(df: pd.DataFrame, window: int = 60, max_lag: int = 30, thresh: float = 0.4, output_dir: Path = Path('.')) -> Optional[pd.Timestamp]:
    ...
    onset = None
    if not res.empty:
        res.to_json(output_dir / 'tvccf_peaks.json', orient='records', date_format='iso')
...
def compute_sliding_granger(df: pd.DataFrame, window: int = 60, maxlag: int = 1, alpha: float = 0.05, output_dir: Path = Path('.')) -> Optional[pd.Timestamp]:
    ...
    onset = None
    if not res.empty:
        res.to_json(output_dir / 'granger_causality.json', orient='records', date_format='iso')
...
def compute_change_point_correlation(df: pd.DataFrame, corr_window: int = 7, output_dir: Path = Path('.')) -> Optional[pd.Timestamp]:
    ...
    if not corr_df.empty:
        corr_df.to_json(output_dir / 'correlation_change_points.json', orient='records', date_format='iso')
        plt.plot(corr_df['timestamp'], corr_df['r'], label='Correlation')
        if cp_idx is not None:
            plt.axvline(corr_df['timestamp'][cp_idx], color='red', linestyle='--', label='Change')
        plt.legend()
...
def compute_wavelet_coherence(df: pd.DataFrame, output_dir: Path = Path('.')) -> Optional[pd.Timestamp]:
    ...
    plt.imshow(coherence, aspect='auto', origin='lower', extent=[0, len(df), scales[0], scales[-1]])
...
def compute_tvp_regression(df: pd.DataFrame, window: int = 60, threshold: float = 0.0, output_dir: Path = Path('.')) -> Optional[pd.Timestamp]:
    ...
    if not df_coef.empty:
        df_coef.to_json(output_dir / 'tvp_regression.json', orient='records', date_format='iso')
...
def _mutual_information(x: np.ndarray, y: np.ndarray, bins: int = 10) -> float:
    ...
    p_y = p_xy.sum(axis=0)
    nz = p_xy > 0
    return float((p_xy[nz] * np.log(p_xy[nz] / (p_x[:, None] * p_y[None, :])[nz])).sum())


def compute_mutual_information(df: pd.DataFrame, window: int = 60, bins: int = 10, threshold: float = 0.1, output_dir: Path = Path('.')) -> Optional[pd.Timestamp]:
    """Sliding-window mutual information between rainfall and movement."""
    records = []
    for i in range(window, len(df)):
        sub = df.iloc[i - window:i]
        mi = _mutual_information(sub['rainfall_mm'].values, sub['movement_mm'].values, bins=bins)
        records.append({'timestamp': df['timestamp'].iloc[i], 'mi': float(mi)})
    mi_df = pd.DataFrame(records)
    onset = None
    if not mi_df.empty:
        mi_df.to_json(output_dir / 'mutual_information.json', orient='records', date_format='iso')
...
def compute_hmm_regime(df: pd.DataFrame, n_states: int = 2, output_dir: Path = Path('.')) -> Optional[pd.Timestamp]:
    ...
    plt.clf()
    pd.DataFrame({'timestamp': df['timestamp'], 'state': states}).to_json(output_dir / 'hmm_states.json', orient='records', date_format='iso')
    return onset
...
def analyze_movement_rain_temp(output_dir: Path = Path("analysis_outputs")) -> None:
    ...
    summary = {
        ...
        "granger_causality": granger_date.strftime('%Y-%m-%d') if granger_date else None,
        "correlation_change_point": cp_date.strftime('%Y-%m-%d') if cp_date else None,
        "wavelet_coherence": wavelet_date.strftime('%Y-%m-%d') if wavelet_date else None,
